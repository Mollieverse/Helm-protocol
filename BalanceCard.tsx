...
export function BalanceCard({ balance, loading, refresh, price }: Props) {
  const { connected, publicKey } = useWallet();
  const positive = price.change24h >= 0;
  const usdValue = balance != null ? (balance * price.current).toFixed(2) : null;

  return (
    <div className="relative rounded-2xl overflow-hidden border border-border glow-purple animate-fade-in">
      {/* gradient backdrop */}
      ...
      <div className="relative p-5 sm:p-6">
        ...
        <div className="flex items-start justify-between mb-4">
          <div>
            <p className="text-xs text-muted uppercase tracking-widest font-mono mb-1">
              Portfolio Balance
            </p>
            {connected && publicKey && (
          ...
          <button
            onClick={refresh}
            className={`p-1.5 rounded-lg hover:bg-card-2 text-muted hover:text-text transition-all ${loading ? 'animate-spin' : ''}`}
          >
        ...
        {!connected ? (
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted">Connect wallet to view balance</p>
          </div>
        ) : loading ? (
          ...
          <div className="space-y-1">
            <div className="flex items-end gap-3">
              <span className="text-4xl sm:text-5xl font-bold tracking-tight text-glow-green">
                {balance?.toFixed(4) ?? '0.0000'}
              </span>
              <span className="text-lg text-muted mb-1">SOL</span>
            </div>
            <p className="text-xl text-muted font-mono">
              ≈ <span className="text-text">${usdValue ?? '0.00'}</span>
